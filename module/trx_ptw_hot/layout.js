...
var m_trx_ptw_hot;
var m_trx_ptw_hot_master;
var m_trx_ptw_hot_master_add;
var	m_trx_ptw_hot_detail_jenis_pekerjaan;
var	m_trx_ptw_hot_detail_sbak;
var	m_trx_ptw_hot_detail_alat_pelindung_diri;
var	m_trx_ptw_hot_detail_safety_checklist;
var	m_trx_ptw_hot_detail_uji_kandungan_gas;
var	m_trx_ptw_hot_detail_validasi;

var m_trx_ptw_hot_id_ptw	= '';
var m_trx_ptw_hot_jenis_ptw = '1';
var m_trx_ptw_hot_ha_level	= 0;

var m_trx_ptw_hot_store_jsa = new Ext.data.SimpleStore({
		fields		: ['id','name']
	,	url			: _g_root +'/module/trx_ptw/data_ref_jsa.jsp'
	,	autoLoad	: false
});

function ptw_hot_master_on_select_load_detail()
{
	if (typeof m_trx_ptw_hot_master == 'undefined'
	||  m_trx_ptw_hot_id_ptw == '') {
		return;
	}

	m_trx_ptw_hot_detail_jenis_pekerjaan.do_refresh();
	m_trx_ptw_hot_detail_sbak.do_refresh();
	m_trx_ptw_hot_detail_alat_pelindung_diri.do_refresh();
	m_trx_ptw_hot_detail_safety_checklist.do_refresh();
	m_trx_ptw_hot_detail_uji_kandungan_gas.do_refresh();
	m_trx_ptw_hot_detail_validasi.do_refresh();
}

function M_TrxPTWHotMasterAdd(title)
{
	this.title 			= title;
	this.dml_type		= 0;
	this.id_ptw			= '';
	...
	this.panel_user = new Ext.form.FormPanel({
		...
		,	items			: [
				...
				,	this.form_nama_petugas_isolasi_listrik
				,	this.form_jabatan_petugas_isolasi_listrik
				,	this.form_nama_petugas_uji_kandungan_gas
				,	this.form_jabatan_petugas_uji_kandungan_gas
	...
	{
		m_trx_ptw_hot.panel.layout.setActiveItem(0);
	}
	...
	this.do_save = function()
	{
		...
		,	success : function (response) {
				...
				m_trx_ptw_hot_master.store.reload();
				m_trx_ptw_hot.panel.layout.setActiveItem(0);
			}
		,	scope	: this
		});
...
function M_TrxPTWHotMaster(title)
{
	this.title	= title;
	this.dml_type	= 0;
	...
	this.store = new Ext.data.ArrayStore({
			fields	: this.record
		,	url	: _g_root +'/module/trx_ptw_hot/data_list_ptw_hot_master.jsp'
		,	autoLoad: false
	});
	...
	this.sm = new Ext.grid.RowSelectionModel({
		...
		,	listeners	: {
			...
			,	selectionchange	: function(sm) {
					...
					if (data.length) {
						m_trx_ptw_hot_id_ptw = data[0].data['id_ptw'];
					...
					ptw_hot_master_on_select_load_detail();
				}
			}
	});
	...
	this.do_edit = function()
	{
		...
		m_trx_ptw_hot_master_add.do_edit(data.get('id_ptw'));
		m_trx_ptw_hot.panel.layout.setActiveItem(1);
	}

	this.do_add = function()
	{
		m_trx_ptw_hot_master_add.do_add();
		m_trx_ptw_hot.panel.layout.setActiveItem(1);
	}
	...
	{
		var data	= this.sm.getSelected();
		var url		= _g_root +'/module/lap_ptw/export_hwp.jsp';
...
function M_TrxPTWHotDetailJenisPekerjaan(title)
{
	this.title		= title;
	this.dml_type	= 0;
	...
	this.record = new Ext.data.Record.create([
		{
			...
			name	: 'id_jenis_pekerjaan_ptw'
		},{
			name	: 'id_jenis_pekerjaan_ptw_old'
		}]);

	this.store = new Ext.data.ArrayStore({
			fields	: this.record
		,	url	: _g_root +'/module/trx_ptw/data_ptw_detail_jenis_pekerjaan.jsp'
		,	autoLoad: false
		});

	this.store_jenis_pekerjaan_ptw = new Ext.data.SimpleStore({
			fields	: ['id','name']
		,	url		: _g_root +'/module/trx_ptw/data_ref_jenis_pekerjaan.jsp'
		,	autoLoad: false
		,	idIndex	: 0
		});

	this.form_jenis_pekerjaan_ptw = new Ext.form.ComboBox({
			store			: this.store_jenis_pekerjaan_ptw
		,	valueField		: 'id'
		,	displayField	: 'name'
		,	mode			: 'local'
		,	allowBlank		: false
	...
	this.columns = [
			new Ext.grid.RowNumberer()
		,	{ id		: 'id_jenis_pekerjaan_ptw'
			, header	: 'Jenis Pekerjaan'
			, dataIndex	: 'id_jenis_pekerjaan_ptw'
			, width		: 300
			, editor	: this.form_jenis_pekerjaan_ptw
			, renderer	: combo_renderer(this.form_jenis_pekerjaan_ptw)
			}
		];
	...
	this.sm = new Ext.grid.RowSelectionModel({
		...
		,	listeners	: {
			...
			,	selectionchange	: function(sm) {
					var data = sm.getSelections();
					if (data.length && m_trx_ptw_hot_ha_level == 4) {
	...
	this.do_save = function(record)
	{
		Ext.Ajax.request({
			...
			,	id_jenis_pekerjaan_ptw		: record.data['id_jenis_pekerjaan_ptw']
			,	id_jenis_pekerjaan_ptw_old	: record.data['id_jenis_pekerjaan_ptw_old']
			,   dml_type					: this.dml_type
			}
		,	url	: _g_root +'/module/trx_ptw/submit_ptw_detail_jenis_pekerjaan.jsp'
		,	waitMsg	: 'Mohon Tunggu ...'
		,	success :
				function (response)
				{
	...
	this.do_load = function()
	{
		this.store_jenis_pekerjaan_ptw.load({
			callback	: function(){
				this.store.load({
					params		: {
						id_ptw	: m_trx_ptw_hot_id_ptw
...
function M_TrxPTWHotDetailSBAK(title)
{
	this.title	= title;
	this.dml_type	= 0;
...
function M_TrxPTWHotDetailAlatPelindungDiri(title)
{
	this.title	= title;
	this.dml_type	= 0;
...
function M_TrxPTWHotDetailSafetyChecklist(title)
{
	this.title	= title;
	this.dml_type	= 0;
...
function M_TrxPTWHotDetailUjiKandunganGas(title)
{
	this.title	= title;
	this.dml_type	= 0;
	...
	this.record = new Ext.data.Record.create([
		{
			...
			name	: 'id_uji_kandungan_gas_ptw'
		},{
			name	: 'id_uji_kandungan_gas_ptw_old'
		},{
			name	: 'hasil'
		}]);
	...
	this.store = new Ext.data.ArrayStore({
			fields	: this.record
		,	url	: _g_root +'/module/trx_ptw/data_ptw_detail_uji_kandungan_gas.jsp'
		,	autoLoad: false
		});

	this.store_uji_kandungan_gas_ptw = new Ext.data.SimpleStore({
			fields	: ['id','name']
		,	url		: _g_root +'/module/trx_ptw/data_ref_uji_kandungan_gas.jsp'
		,	autoLoad: false
		,	idIndex	: 0
		});

	this.form_uji_kandungan_gas_ptw = new Ext.form.ComboBox({
			store			: this.store_uji_kandungan_gas_ptw
		,	valueField		: 'id'
		,	displayField	: 'name'
		,	mode			: 'local'
		,	allowBlank		: false
	...
	this.form_hasil = new Ext.form.TextField({});
	
	this.columns = [
			new Ext.grid.RowNumberer()
		,	{ id		: 'id_uji_kandungan_gas_ptw'
			, header	: 'Gas Item'
			, dataIndex	: 'id_uji_kandungan_gas_ptw'
			, width		: 300
			, editor	: this.form_uji_kandungan_gas_ptw
			, renderer	: combo_renderer(this.form_uji_kandungan_gas_ptw)
			}
		,	{ id		: 'hasil'
			, header	: 'Hasil'
			, dataIndex	: 'hasil'
			, width		: 300
			, editor	: this.form_hasil
			}
		];
	...
	this.do_save = function(record)
	{
		Ext.Ajax.request({
			params  : {
				id_ptw							: record.data['id_ptw']
			,	id_uji_kandungan_gas_ptw		: record.data['id_uji_kandungan_gas_ptw']
			,	id_uji_kandungan_gas_ptw_old	: record.data['id_uji_kandungan_gas_ptw_old']
			,	hasil							: record.data['hasil']
			,   dml_type						: this.dml_type
			}
		,	url	: _g_root +'/module/trx_ptw/submit_ptw_detail_uji_kandungan_gas.jsp'
		,	waitMsg	: 'Mohon Tunggu ...'
		,	success :
				function (response)
				{
	...
	this.do_load = function()
	{
		this.store_uji_kandungan_gas_ptw.load({
			callback	: function(){
				this.store.load({
					params		: {
						id_ptw	: m_trx_ptw_hot_id_ptw
...
function M_TrxPTWHotDetailValidasi(title)
{
	this.title	= title;
	this.dml_type	= 0;
...
function M_TrxPTWHot()
{
	m_trx_ptw_hot_master						= new M_TrxPTWHotMaster('Hot Work Permit');
	m_trx_ptw_hot_master_add					= new M_TrxPTWHotMasterAdd('Entry / Edit Data Hot Work Permit');
	m_trx_ptw_hot_detail_jenis_pekerjaan		= new M_TrxPTWHotDetailJenisPekerjaan('Jenis Pekerjaan');
	m_trx_ptw_hot_detail_sbak					= new M_TrxPTWHotDetailSBAK('Sumber Bahaya Alat/Kegiatan');
	m_trx_ptw_hot_detail_alat_pelindung_diri	= new M_TrxPTWHotDetailAlatPelindungDiri('Alat Pelindung Diri');
	m_trx_ptw_hot_detail_safety_checklist		= new M_TrxPTWHotDetailSafetyChecklist('Safety Checklist');
	m_trx_ptw_hot_detail_uji_kandungan_gas		= new M_TrxPTWHotDetailUjiKandunganGas('Uji Kandungan Gas');
	m_trx_ptw_hot_detail_validasi				= new M_TrxPTWHotDetailValidasi('Validasi');

	this.panel_ptw_hot_detail = new Ext.TabPanel({
			autoScroll		: true
		...
		,	items			: [
				m_trx_ptw_hot_detail_jenis_pekerjaan.grid
			,	m_trx_ptw_hot_detail_sbak.grid
			,	m_trx_ptw_hot_detail_alat_pelindung_diri.grid
			,	m_trx_ptw_hot_detail_safety_checklist.grid
			,	m_trx_ptw_hot_detail_uji_kandungan_gas.grid
			,	m_trx_ptw_hot_detail_validasi.grid
			]
	});
	
	this.panel_ptw_hot = new Ext.Panel({
			layout			: 'border'
		,	bodyBorder		: false
		,	defaults		: {
				loadMask		: {msg: 'Pemuatan...'}
		...
		,	items			: [
				m_trx_ptw_hot_master.grid
			,	this.panel_ptw_hot_detail
			]
	});

	this.panel = new Ext.Panel({
			id				: 'trx_ptw_hot_panel'
		,	layout			: 'card'
		,	activeItem		: 0
		,	autoWidth		: true
		,	autoScroll		: true
		,	items			: [
				this.panel_ptw_hot
			,	m_trx_ptw_hot_master_add.panel
			]
	});

	this.do_refresh = function(ha_level)
	{
		m_trx_ptw_hot_ha_level	= ha_level;
		m_trx_ptw_hot_id_ptw 	= '';
		
		m_trx_ptw_hot_store_jsa.load();
		
		m_trx_ptw_hot_master.do_refresh();
	}
}

m_trx_ptw_hot = new M_TrxPTWHot();

//@ sourceURL=trx_ptw_hot.layout.js
