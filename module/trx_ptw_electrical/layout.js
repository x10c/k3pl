...
var m_trx_ptw_electrical;
var m_trx_ptw_electrical_master;
var m_trx_ptw_electrical_master_add;
var	m_trx_ptw_electrical_detail_alat_pelindung_diri;
var	m_trx_ptw_electrical_detail_safety_checklist;
var	m_trx_ptw_electrical_detail_uji_kandungan_gas;
var	m_trx_ptw_electrical_detail_validasi;

var m_trx_ptw_electrical_id_ptw	= '';
var m_trx_ptw_electrical_jenis_ptw = '5';
var m_trx_ptw_electrical_ha_level	= 0;

var m_trx_ptw_electrical_store_jsa = new Ext.data.SimpleStore({
		fields		: ['id','name']
	,	url			: _g_root +'/module/trx_ptw/data_ref_jsa.jsp'
	,	autoLoad	: false
});

function ptw_electrical_master_on_select_load_detail()
{
	if (typeof m_trx_ptw_electrical_master == 'undefined'
	||  m_trx_ptw_electrical_id_ptw == '') {
		return;
	}

	m_trx_ptw_electrical_detail_alat_pelindung_diri.do_refresh();
	m_trx_ptw_electrical_detail_safety_checklist.do_refresh();
	m_trx_ptw_electrical_detail_uji_kandungan_gas.do_refresh();
	m_trx_ptw_electrical_detail_validasi.do_refresh();
}

function M_TrxPTWElectricalMasterAdd(title)
{
	this.title 			= title;
	this.dml_type		= 0;
	this.id_ptw			= '';
	...
	this.form_dokumen_pendukung = new Ext.form.ComboBox({
		...
		,	store			: m_trx_ptw_electrical_store_jsa
		,	valueField		: 'id'
		,	displayField	: 'name'
		,	mode			: 'local'
		,	allowBlank		: false
	...
	{
		m_trx_ptw_electrical.panel.layout.setActiveItem(0);
	}
	...
	this.do_save = function()
	{
		...
		,	params	: {
				...
				,	id_ptw								: m_trx_ptw_electrical_id_ptw
				,	jenis_ptw							: m_trx_ptw_electrical_jenis_ptw
		...
		,	success : function (response) {
				...
				m_trx_ptw_electrical_master.store.reload();
				m_trx_ptw_electrical.panel.layout.setActiveItem(0);
			}
		,	scope	: this
		});
	...
	{
		if (m_trx_ptw_electrical_ha_level <= 1) {
...
function M_TrxPTWElectricalMaster(title)
{
	this.title	= title;
	this.dml_type	= 0;
	...
	this.store = new Ext.data.ArrayStore({
			fields	: this.record
		,	url	: _g_root +'/module/trx_ptw_electrical/data_list_ptw_electrical_master.jsp'
		,	autoLoad: false
	});
	...
	this.sm = new Ext.grid.RowSelectionModel({
		...
		,	listeners	: {
			...
			,	selectionchange	: function(sm) {
					...
					if (data.length) {
						m_trx_ptw_electrical_id_ptw = data[0].data['id_ptw'];
					...
					ptw_electrical_master_on_select_load_detail();
				}
			}
	});
	...
	this.do_edit = function()
	{
		...
		m_trx_ptw_electrical_master_add.do_edit(data.get('id_ptw'));
		m_trx_ptw_electrical.panel.layout.setActiveItem(1);
	}

	this.do_add = function()
	{
		m_trx_ptw_electrical_master_add.do_add();
		m_trx_ptw_electrical.panel.layout.setActiveItem(1);
	}
	...
	this.do_print = function()
	{
		...
		form.setAttribute('action', _g_root +'/module/lap_ptw/export_ewp.jsp');
	...
	this.do_load = function()
	{
		...
		if (m_trx_ptw_electrical_ha_level == 4) {
			load_type = 'all';
		}
...
function M_TrxPTWElectricalDetailAlatPelindungDiri(title)
{
	this.title	= title;
	this.dml_type	= 0;
	...
	this.sm = new Ext.grid.RowSelectionModel({
		...
		,	listeners	: {
			...
			,	selectionchange	: function(sm) {
					var data = sm.getSelections();
					if (data.length && m_trx_ptw_electrical_ha_level == 4) {
	...
	this.do_add = function()
	{
		if (m_trx_ptw_electrical_id_ptw == '') {
	...
	{
		if (m_trx_ptw_electrical_ha_level >= 3) {
			this.dml_type = 3;
			return true;
		}
		return false;
	...
	this.do_refresh = function()
	{
		if (m_trx_ptw_electrical_ha_level < 2) {
			this.btn_add.setDisabled(true);
			this.btn_del.setDisabled(true);
		} else {
			this.btn_add.setDisabled(false);
...
function M_TrxPTWElectricalDetailSafetyChecklist(title)
{
	this.title	= title;
	this.dml_type	= 0;
...
function M_TrxPTWElectricalDetailUjiKandunganGas(title)
{
	this.title	= title;
	this.dml_type	= 0;
...
function M_TrxPTWElectricalDetailValidasi(title)
{
	this.title	= title;
	this.dml_type	= 0;
...
function M_TrxPTWElectrical()
{
	m_trx_ptw_electrical_master						= new M_TrxPTWElectricalMaster('Electrical Work Permit');
	m_trx_ptw_electrical_master_add					= new M_TrxPTWElectricalMasterAdd('Entry / Edit Data Electrical Work Permit');
	m_trx_ptw_electrical_detail_alat_pelindung_diri	= new M_TrxPTWElectricalDetailAlatPelindungDiri('Alat Pelindung Diri');
	m_trx_ptw_electrical_detail_safety_checklist	= new M_TrxPTWElectricalDetailSafetyChecklist('Safety Checklist');
	m_trx_ptw_electrical_detail_uji_kandungan_gas	= new M_TrxPTWElectricalDetailUjiKandunganGas('Uji Kandungan Gas');
	m_trx_ptw_electrical_detail_validasi			= new M_TrxPTWElectricalDetailValidasi('Validasi');

	this.panel_ptw_electrical_detail = new Ext.TabPanel({
			autoScroll		: true
		...
		,	items			: [
				m_trx_ptw_electrical_detail_alat_pelindung_diri.grid
			,	m_trx_ptw_electrical_detail_safety_checklist.grid
			,	m_trx_ptw_electrical_detail_uji_kandungan_gas.grid
			,	m_trx_ptw_electrical_detail_validasi.grid
			]
	});
	
	this.panel_ptw_electrical = new Ext.Panel({
			layout			: 'border'
		,	bodyBorder		: false
		,	defaults		: {
				loadMask		: {msg: 'Pemuatan...'}
		...
		,	items			: [
				m_trx_ptw_electrical_master.grid
			,	this.panel_ptw_electrical_detail
			]
	});

	this.panel = new Ext.Panel({
			id				: 'trx_ptw_electrical_panel'
		,	layout			: 'card'
		,	activeItem		: 0
		,	autoWidth		: true
		,	autoScroll		: true
		,	items			: [
				this.panel_ptw_electrical
			,	m_trx_ptw_electrical_master_add.panel
			]
	});

	this.do_refresh = function(ha_level)
	{
		m_trx_ptw_electrical_ha_level	= ha_level;
		m_trx_ptw_electrical_id_ptw 	= '';
		
		m_trx_ptw_electrical_store_jsa.load();
		
		m_trx_ptw_electrical_master.do_refresh();
	}
}

m_trx_ptw_electrical = new M_TrxPTWElectrical();

//@ sourceURL=trx_ptw_electrical.layout.js
