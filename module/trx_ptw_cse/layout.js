...
var m_trx_ptw_cse;
var m_trx_ptw_cse_master;
var m_trx_ptw_cse_master_add;
var	m_trx_ptw_cse_detail_alat_pelindung_diri;
var	m_trx_ptw_cse_detail_safety_checklist;
var	m_trx_ptw_cse_detail_uji_kandungan_gas;
var	m_trx_ptw_cse_detail_validasi;

var m_trx_ptw_cse_id_ptw	= '';
var m_trx_ptw_cse_jenis_ptw = '3';
var m_trx_ptw_cse_ha_level	= 0;

var m_trx_ptw_cse_store_jsa = new Ext.data.SimpleStore({
		fields		: ['id','name']
	,	url			: _g_root +'/module/trx_ptw/data_ref_jsa.jsp'
	,	autoLoad	: false
});

function ptw_cse_master_on_select_load_detail()
{
	if (typeof m_trx_ptw_cse_master == 'undefined'
	||  m_trx_ptw_cse_id_ptw == '') {
		return;
	}

	m_trx_ptw_cse_detail_alat_pelindung_diri.do_refresh();
	m_trx_ptw_cse_detail_safety_checklist.do_refresh();
	m_trx_ptw_cse_detail_uji_kandungan_gas.do_refresh();
	m_trx_ptw_cse_detail_validasi.do_refresh();
}

function M_TrxPTWCSEMasterAdd(title)
{
	this.title 			= title;
	this.dml_type		= 0;
	this.id_ptw			= '';
	...
	{
		m_trx_ptw_cse.panel.layout.setActiveItem(0);
	}
	...
	this.do_save = function()
	{
		...
		,	params	: {
				...
				,	id_ptw								: m_trx_ptw_cse_id_ptw
				,	jenis_ptw							: m_trx_ptw_cse_jenis_ptw
		...
		,	success : function (response) {
				...
				m_trx_ptw_cse_master.store.reload();
				m_trx_ptw_cse.panel.layout.setActiveItem(0);
			}
		,	scope	: this
		});
...
function M_TrxPTWCSEMaster(title)
{
	this.title	= title;
	this.dml_type	= 0;
	...
	this.store = new Ext.data.ArrayStore({
			fields	: this.record
		,	url	: _g_root +'/module/trx_ptw_cse/data_list_ptw_cse_master.jsp'
		,	autoLoad: false
	});
	...
	this.sm = new Ext.grid.RowSelectionModel({
		...
		,	listeners	: {
			...
			,	selectionchange	: function(sm) {
					...
					if (data.length) {
						m_trx_ptw_cse_id_ptw = data[0].data['id_ptw'];
					...
					ptw_cse_master_on_select_load_detail();
				}
			}
	});
	...
	this.do_edit = function()
	{
		...
		m_trx_ptw_cse_master_add.do_edit(data.get('id_ptw'));
		m_trx_ptw_cse.panel.layout.setActiveItem(1);
	}

	this.do_add = function()
	{
		m_trx_ptw_cse_master_add.do_add();
		m_trx_ptw_cse.panel.layout.setActiveItem(1);
	}
...
function M_TrxPTWCSEDetailAlatPelindungDiri(title)
{
	this.title	= title;
	this.dml_type	= 0;
	...
	this.sm = new Ext.grid.RowSelectionModel({
		...
		,	listeners	: {
			...
			,	selectionchange	: function(sm) {
					var data = sm.getSelections();
					if (data.length && m_trx_ptw_cse_ha_level == 4) {
...
function M_TrxPTWCSEDetailSafetyChecklist(title)
{
	this.title	= title;
	this.dml_type	= 0;
...
function M_TrxPTWCSEDetailUjiKandunganGas(title)
{
	this.title	= title;
	this.dml_type	= 0;
...
function M_TrxPTWCSEDetailValidasi(title)
{
	this.title	= title;
	this.dml_type	= 0;
...
function M_TrxPTWCSE()
{
	m_trx_ptw_cse_master						= new M_TrxPTWCSEMaster('Confined Space Entry Permit');
	m_trx_ptw_cse_master_add					= new M_TrxPTWCSEMasterAdd('Entry / Edit Data Confined Space Entry Permit');
	m_trx_ptw_cse_detail_alat_pelindung_diri	= new M_TrxPTWCSEDetailAlatPelindungDiri('Alat Pelindung Diri');
	m_trx_ptw_cse_detail_safety_checklist		= new M_TrxPTWCSEDetailSafetyChecklist('Safety Checklist');
	m_trx_ptw_cse_detail_uji_kandungan_gas		= new M_TrxPTWCSEDetailUjiKandunganGas('Uji Kandungan Gas');
	m_trx_ptw_cse_detail_validasi				= new M_TrxPTWCSEDetailValidasi('Validasi');

	this.panel_ptw_cse_detail = new Ext.TabPanel({
			autoScroll		: true
		...
		,	items			: [
				m_trx_ptw_cse_detail_alat_pelindung_diri.grid
			,	m_trx_ptw_cse_detail_safety_checklist.grid
			,	m_trx_ptw_cse_detail_uji_kandungan_gas.grid
			,	m_trx_ptw_cse_detail_validasi.grid
			]
	});
	
	this.panel_ptw_cse = new Ext.Panel({
			layout			: 'border'
		,	bodyBorder		: false
		,	defaults		: {
				loadMask		: {msg: 'Pemuatan...'}
		...
		,	items			: [
				m_trx_ptw_cse_master.grid
			,	this.panel_ptw_cse_detail
			]
	});

	this.panel = new Ext.Panel({
			id				: 'trx_ptw_cse_panel'
		,	layout			: 'card'
		,	activeItem		: 0
		,	autoWidth		: true
		,	autoScroll		: true
		,	items			: [
				this.panel_ptw_cse
			,	m_trx_ptw_cse_master_add.panel
			]
	});

	this.do_refresh = function(ha_level)
	{
		m_trx_ptw_cse_ha_level	= ha_level;
		m_trx_ptw_cse_id_ptw 	= '';
		
		m_trx_ptw_cse_store_jsa.load();
		
		m_trx_ptw_cse_master.do_refresh();
	}
}

m_trx_ptw_cse = new M_TrxPTWCSE();

//@ sourceURL=trx_ptw_cse.layout.js
